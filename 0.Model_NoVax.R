...
seir_noVax <- function(times,init_novax,parms_novax){
  ...
  with(as.list(c(init_novax,parms_novax)), {
    ...
    dS <- -beta*(S/n)*((E*presymp_infect) + (A*asymp_infect) + I + (EV*presymp_infect) + (AV*asymp_infect) + IV) * (1-(mask_eff*mask_prop))
    dE <-  beta*(S/n)*((E*presymp_infect) + (A*asymp_infect) + I + (EV*presymp_infect) + (AV*asymp_infect) + IV) * (1-(mask_eff*mask_prop))- l*E
    ...
    s.flow.out <- -beta*(S/n)*((E*presymp_infect) + (A*asymp_infect) + I + (EV*presymp_infect) + (AV*asymp_infect) + IV) * (1-(mask_eff*mask_prop))
    e.flow.in <- beta*(S/n)*((E*presymp_infect) + (A*asymp_infect) + I + (EV*presymp_infect) + (AV*asymp_infect) + IV) * (1-(mask_eff*mask_prop))
    e.flow.out <- - l*E
...
# Parameters 
parms_novax <- cbind(beta, presymp_infect, asymp_infect, p_symp, p_asymp, mask_eff, mask_prop, n, l, h, p, g, cfr)
...
model_noVax <- as.data.frame(ode(init_novax, times, seir_noVax, parms_novax)) 
...
#plot(times, model_noVax$I, type = 'l')
#plot(times, model_noVax$A, type = 'l')
